...
public class StorageCalculator {
	...
	public StorageCalculateDto calculateYearlyRevenue(StorageInfo storageInfo, DealInfo dealInfo,
		...
		BigDecimal avgVolume = commonHelper.getStorageAverageVolume(storageInfo.getStorageYearlyDataInfos());
		StorageCalculateDto calculateDto = null;
		if (avgVolume != null && avgVolume.compareTo(new BigDecimal(0)) != 0) {
			...
			calculateDto = calculateBenchMarkDealRevenue(storageInfo, dealInfo, assessmentDealTerm, referenceCountryFactor,
					...
					calculateDto == null ? new StorageCalculateDto() : calculateDto);
		}
	 ...
	 */
	@Transactional
	private StorageCalculateDto calculatePastDealRevenue(StorageInfo storageInfo, DealInfo dealInfo,
			TowerSpecificBandInfo towerSpecificBandInfo, Integer assessmentDealTerm, List<CountryFactorInfo> countryFactors,
			BigDecimal referenceCountryFactor, BigDecimal avgVolume, StorageCalculateDto calculateDto) {

		Specification<StorageInfo> specification = StorageSpecification.specificationForStorage(
		...
		log.info("Storage dealResults.size :" + pastDealResults.size());
		...
		if (!CollectionUtils.isEmpty(pastDealResults)) {
			...
			commonHelper.applyFxRatesAndCountryFactor(pastDealResults, countryFactors, dealInfo.getCurrency(),
					dealInfo.getCountry(), referenceCountryFactor, STORAGE);
			calculateDto = prepareCalculateDtoForPastDeal(pastDealResults, storageInfo,
					new StorageCalculateDto());
	 ...
	 * @return
	 */
	private StorageCalculateDto prepareCalculateDtoForPastDeal(List<StorageInfo> pastDealResults,
			StorageInfo storageInfo, StorageCalculateDto calculateDto) {
	 ...
	 * @param pastDealResults
	 */
	private void setPrice(StorageInfo storageInfo, StorageCalculateDto calculateDto,
					...
					if (!CollectionUtils.isEmpty(yearlyDataInfo.getStorageUnitPriceInfo())
						...
						if (yearlyUnitPriceMap.containsKey(yearlyDataInfo.getYear())) {
							yearlyUnitPriceMap.put(yearlyDataInfo.getYear(), yearlyUnitPriceMap
									.get(yearlyDataInfo.getYear()).add(unitPriceInfo.getStorageVolumeUnitPrice()));
						} else {
							yearlyUnitPriceMap.put(yearlyDataInfo.getYear(), unitPriceInfo.getStorageVolumeUnitPrice());
						}
					}
				}
			}
	...
	private Map<Integer, BigDecimal> getYearlyRevenue(Map<Integer, BigDecimal> yearlyUnitPriceMap,
		...
		for (StorageYearlyDataInfo yearlyDataInfo : storageInfo.getStorageYearlyDataInfos()) {
			yearlyRevenueMap.put(yearlyDataInfo.getYear(), yearlyUnitPriceMap.get(yearlyDataInfo.getYear())
					.multiply(new BigDecimal(yearlyDataInfo.getStorageVolume())));
		}
	...
	private BigDecimal getConsolidatedAvgUnitPrice(Map<Integer, BigDecimal> yearlyRevenueMap,
		...
		for (Map.Entry<Integer, BigDecimal> entry : yearlyRevenueMap.entrySet()) {
			...
			totalDevices = totalDevices + storageInfo.getStorageYearlyDataInfos().get(i).getStorageVolume();
			i++;
		}
		if(totalDevices != 0){
	...
	@Transactional
	private StorageCalculateDto calculateBenchMarkDealRevenue(StorageInfo storageInfo, DealInfo dealInfo,
			Integer assessmentDealTerm, BigDecimal referenceCountryFactor, List<CountryFactorInfo> countryFactors,
			BigDecimal avgVolume, StorageCalculateDto calculateDto) {

		Specification<StorageInfo> specification = StorageSpecification.specificationForStorageBenchmark(
				storageInfo.isOffshoreAllowed(), storageInfo.getServiceWindowSla(), storageInfo.isIncludeHardware(),
				avgVolume);
		List<StorageInfo> benchMarkDealResults = storageRepository.findAll(specification);
		log.info("Storage Benchmark dealResults.size :" + benchMarkDealResults.size());
		...
		if (!CollectionUtils.isEmpty(benchMarkDealResults)) {
			...
			commonHelper.applyFxRatesAndCountryFactor(benchMarkDealResults, countryFactors,
					dealInfo.getCurrency(), dealInfo.getCountry(), referenceCountryFactor, STORAGE);
		}

		BigDecimal totalYearlySum = new BigDecimal(0);
		Map<Integer, BigDecimal> yearlyVolumeMap = new HashMap<>();
		for (StorageYearlyDataInfo yearlyDataInfo : storageInfo.getStorageYearlyDataInfos()) {
			BigDecimal totalDevices = new BigDecimal(yearlyDataInfo.getStorageVolume());
	 ...
	 */
	private void setBenchmarkPrice(StorageInfo storageInfo, StorageCalculateDto calculateDto,
			...
			for (StorageYearlyDataInfo yearlyDataInfo : benchmarkStorageInfo.getStorageYearlyDataInfos()) {
				if (!CollectionUtils.isEmpty(yearlyDataInfo.getStorageUnitPriceInfo())) {
					for (StorageUnitPriceInfo unitPriceInfo : yearlyDataInfo.getStorageUnitPriceInfo()) {
						...
						unitPrice = unitPriceInfo.getStorageVolumeUnitPrice();
