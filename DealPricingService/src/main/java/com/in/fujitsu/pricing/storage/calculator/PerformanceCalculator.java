...
public class PerformanceCalculator {
	...
	public PerformanceCalculateDto calculateYearlyRevenue(StorageInfo storageInfo, DealInfo dealInfo,
		...
		BigDecimal avgVolume = commonHelper.getPerformanceAverageVolume(storageInfo.getStorageYearlyDataInfos());
		PerformanceCalculateDto calculateDto = null;
		if (avgVolume != null && avgVolume.compareTo(new BigDecimal(0)) != 0) {
			...
			calculateDto = calculateBenchMarkDealRevenue(storageInfo, dealInfo, assessmentDealTerm, referenceCountryFactor,
					...
					calculateDto == null ? new PerformanceCalculateDto() : calculateDto);
		}
	 ...
	 */
	@Transactional
	private PerformanceCalculateDto calculatePastDealRevenue(StorageInfo storageInfo, DealInfo dealInfo,
			TowerSpecificBandInfo towerSpecificBandInfo, Integer assessmentDealTerm, List<CountryFactorInfo> countryFactors,
			BigDecimal referenceCountryFactor, BigDecimal avgVolume, PerformanceCalculateDto calculateDto) {
		...
		if (!CollectionUtils.isEmpty(pastDealResults)) {
			...
			commonHelper.applyFxRatesAndCountryFactor(pastDealResults, countryFactors, dealInfo.getCurrency(),
					dealInfo.getCountry(), referenceCountryFactor, PERFORMANCE);
			calculateDto = prepareCalculateDtoForPastDeal(pastDealResults, storageInfo,
					new PerformanceCalculateDto());
	 ...
	 * @return
	 */
	private PerformanceCalculateDto prepareCalculateDtoForPastDeal(List<StorageInfo> pastDealResults,
			StorageInfo storageInfo, PerformanceCalculateDto calculateDto) {
	 ...
	 * @param pastDealResults
	 */
	private void setPrice(StorageInfo storageInfo, PerformanceCalculateDto calculateDto,
					...
					if (!CollectionUtils.isEmpty(yearlyDataInfo.getStorageUnitPriceInfo())
						...
						if (yearlyUnitPriceMap.containsKey(yearlyDataInfo.getYear())) {
							yearlyUnitPriceMap.put(yearlyDataInfo.getYear(), yearlyUnitPriceMap
									.get(yearlyDataInfo.getYear()).add(unitPriceInfo.getPerformanceUnitPrice()));
						} else {
							yearlyUnitPriceMap.put(yearlyDataInfo.getYear(), unitPriceInfo.getPerformanceUnitPrice());
						}
					}
				}
			}
	...
	private Map<Integer, BigDecimal> getYearlyRevenue(Map<Integer, BigDecimal> yearlyUnitPriceMap,
		...
		for (StorageYearlyDataInfo yearlyDataInfo : storageInfo.getStorageYearlyDataInfos()) {
			yearlyRevenueMap.put(yearlyDataInfo.getYear(), yearlyUnitPriceMap.get(yearlyDataInfo.getYear())
					.multiply(new BigDecimal(yearlyDataInfo.getPerformanceStorage())));
		}
	...
	private BigDecimal getConsolidatedAvgUnitPrice(Map<Integer, BigDecimal> yearlyRevenueMap,
		...
		for (Map.Entry<Integer, BigDecimal> entry : yearlyRevenueMap.entrySet()) {
			...
			totalDevices = totalDevices + storageInfo.getStorageYearlyDataInfos().get(i).getPerformanceStorage();
			i++;
		}
		if(totalDevices != 0){
	...
	@Transactional
	private PerformanceCalculateDto calculateBenchMarkDealRevenue(StorageInfo storageInfo, DealInfo dealInfo,
			Integer assessmentDealTerm, BigDecimal referenceCountryFactor, List<CountryFactorInfo> countryFactors,
			BigDecimal avgVolume, PerformanceCalculateDto calculateDto) {

		Specification<StorageInfo> specification = StorageSpecification.specificationForPerformanceBenchmark(
				storageInfo.isOffshoreAllowed(), storageInfo.getServiceWindowSla(), storageInfo.isIncludeHardware(),
				avgVolume);
		List<StorageInfo> benchMarkDealResults = storageRepository.findAll(specification);
		log.info("Performance Benchmark dealResults.size :" + benchMarkDealResults.size());
		...
		if (!CollectionUtils.isEmpty(benchMarkDealResults)) {
			...
			commonHelper.applyFxRatesAndCountryFactor(benchMarkDealResults, countryFactors,
					dealInfo.getCurrency(), dealInfo.getCountry(), referenceCountryFactor, PERFORMANCE);
		}

		BigDecimal totalYearlySum = new BigDecimal(0);
		Map<Integer, BigDecimal> yearlyVolumeMap = new HashMap<>();
		for (StorageYearlyDataInfo yearlyDataInfo : storageInfo.getStorageYearlyDataInfos()) {
			BigDecimal totalDevices = new BigDecimal(yearlyDataInfo.getPerformanceStorage());
	 ...
	 */
	private void setBenchmarkPrice(StorageInfo storageInfo, PerformanceCalculateDto calculateDto,
			...
			for (StorageYearlyDataInfo yearlyDataInfo : benchmarkStorageInfo.getStorageYearlyDataInfos()) {
				if (!CollectionUtils.isEmpty(yearlyDataInfo.getStorageUnitPriceInfo())) {
					for (StorageUnitPriceInfo unitPriceInfo : yearlyDataInfo.getStorageUnitPriceInfo()) {
						...
						unitPrice = unitPriceInfo.getPerformanceUnitPrice();
