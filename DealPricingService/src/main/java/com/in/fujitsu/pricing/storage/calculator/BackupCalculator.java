...
public class BackupCalculator {
	...
	public BackupCalculateDto calculateYearlyRevenue(StorageInfo storageInfo, DealInfo dealInfo,
		...
		BigDecimal avgVolume = commonHelper.getBackupAverageVolume(storageInfo.getStorageYearlyDataInfos());
		BackupCalculateDto calculateDto = null;
		if (avgVolume != null && avgVolume.compareTo(new BigDecimal(0)) != 0) {
			...
			calculateDto = calculateBenchMarkDealRevenue(storageInfo, dealInfo, assessmentDealTerm, referenceCountryFactor,
					...
					calculateDto == null ? new BackupCalculateDto() : calculateDto);
		}
	 ...
	 */
	@Transactional
	private BackupCalculateDto calculatePastDealRevenue(StorageInfo storageInfo, DealInfo dealInfo,
			TowerSpecificBandInfo towerSpecificBandInfo, Integer assessmentDealTerm, List<CountryFactorInfo> countryFactors,
			BigDecimal referenceCountryFactor, BigDecimal avgVolume, BackupCalculateDto calculateDto) {

		Specification<StorageInfo> specification = StorageSpecification.specificationForBackup(
		...
		log.info("Backup dealResults.size :" + pastDealResults.size());
		...
		if (!CollectionUtils.isEmpty(pastDealResults)) {
			...
			commonHelper.applyFxRatesAndCountryFactor(pastDealResults, countryFactors, dealInfo.getCurrency(),
					dealInfo.getCountry(), referenceCountryFactor, BACKUP);
			calculateDto = prepareCalculateDtoForPastDeal(pastDealResults, storageInfo,
					new BackupCalculateDto());
	 ...
	 * @return
	 */
	private BackupCalculateDto prepareCalculateDtoForPastDeal(List<StorageInfo> pastDealResults,
			StorageInfo storageInfo, BackupCalculateDto calculateDto) {
	 ...
	 * @param pastDealResults
	 */
	private void setPrice(StorageInfo storageInfo, BackupCalculateDto calculateDto,
					...
					if (!CollectionUtils.isEmpty(yearlyDataInfo.getStorageUnitPriceInfo())
						...
						if (yearlyUnitPriceMap.containsKey(yearlyDataInfo.getYear())) {
							yearlyUnitPriceMap.put(yearlyDataInfo.getYear(), yearlyUnitPriceMap
									.get(yearlyDataInfo.getYear()).add(unitPriceInfo.getBackupVolumeUnitPrice()));
						} else {
							yearlyUnitPriceMap.put(yearlyDataInfo.getYear(), unitPriceInfo.getBackupVolumeUnitPrice());
						}
					}
				}
			}
	...
	private Map<Integer, BigDecimal> getYearlyRevenue(Map<Integer, BigDecimal> yearlyUnitPriceMap,
		...
		for (StorageYearlyDataInfo yearlyDataInfo : storageInfo.getStorageYearlyDataInfos()) {
			yearlyRevenueMap.put(yearlyDataInfo.getYear(), yearlyUnitPriceMap.get(yearlyDataInfo.getYear())
					.multiply(new BigDecimal(yearlyDataInfo.getBackupVolume())));
		}
	...
	private BigDecimal getConsolidatedAvgUnitPrice(Map<Integer, BigDecimal> yearlyRevenueMap,
		...
		for (Map.Entry<Integer, BigDecimal> entry : yearlyRevenueMap.entrySet()) {
			...
			totalDevices = totalDevices + storageInfo.getStorageYearlyDataInfos().get(i).getBackupVolume();
			i++;
		}
		if(totalDevices != 0){
	...
	@Transactional
	private BackupCalculateDto calculateBenchMarkDealRevenue(StorageInfo storageInfo, DealInfo dealInfo,
			Integer assessmentDealTerm, BigDecimal referenceCountryFactor, List<CountryFactorInfo> countryFactors,
			BigDecimal avgVolume, BackupCalculateDto calculateDto) {

		Specification<StorageInfo> specification = StorageSpecification.specificationForBenchMarkBackupVolume(
				storageInfo.isOffshoreAllowed(), storageInfo.getServiceWindowSla(), storageInfo.isIncludeHardware(),
				avgVolume);
		List<StorageInfo> benchMarkDealResults = storageRepository.findAll(specification);
		log.info("Backup Benchmark dealResults.size :" + benchMarkDealResults.size());
		...
		if (!CollectionUtils.isEmpty(benchMarkDealResults)) {
			...
			commonHelper.applyFxRatesAndCountryFactor(benchMarkDealResults, countryFactors,
					dealInfo.getCurrency(), dealInfo.getCountry(), referenceCountryFactor, BACKUP);
		}

		BigDecimal totalYearlySum = new BigDecimal(0);
		Map<Integer, BigDecimal> yearlyVolumeMap = new HashMap<>();
		for (StorageYearlyDataInfo yearlyDataInfo : storageInfo.getStorageYearlyDataInfos()) {
			BigDecimal totalDevices = new BigDecimal(yearlyDataInfo.getBackupVolume());
	 ...
	 */
	private void setBenchmarkPrice(StorageInfo storageInfo, BackupCalculateDto calculateDto,
		...
		if (!CollectionUtils.isEmpty(benchMarkDealResults)) {
			...
			for (StorageYearlyDataInfo yearlyDataInfo : benchmarkStorageInfo.getStorageYearlyDataInfos()) {
				if (!CollectionUtils.isEmpty(yearlyDataInfo.getStorageUnitPriceInfo())) {
					for (StorageUnitPriceInfo unitPriceInfo : yearlyDataInfo.getStorageUnitPriceInfo()) {
						...
						unitPrice = unitPriceInfo.getBackupVolumeUnitPrice();
			...
			calculateDto.setBenchDealLowAvgUnitPrice(
					totalLowRevenue.divide(totalYearlySum, 2, BigDecimal.ROUND_CEILING));
